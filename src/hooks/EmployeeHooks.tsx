...
export async function handleAddProfileImage(
  id: string,
...
export async function handleEmployeeCustomization(props: Customize) {
  try {
    if (props.email && (props.salary || props.position || props.PTO)) {
      ...
      toast.error(
        "Please Fill Out Your Email And At Least The Salary, Position, Or PTO Inputs And Try Again!",
      );
    }
  } catch (err) {
    console.error(err);
...
export async function handlePTO(
  listOfUsers: User[],
  PTO: string,
  PTOStartDate: string,
  PTOEndDate: string,
  ...
  try {
    const findUser = listOfUsers.filter(
      (employee: User) => employee.email === cacheEmail,
    )[0];
    ...
    const findEmployee = employeeList.documents.filter(
      (employee: Profile) => employee.email === cacheEmail,
    );
    ...
    if (!PTOEndDay || !PTOStartDate || !Number(PTO)) {
      toast.error(
        "Please make sure all input fields are filled out or are valid.",
      );
      return;
    }
    ...
    if (
      PTOStartYear > PTOEndYear ||
      PTOStartYear < currentYear ||
      PTOEndYear < currentYear ||
      PTOStartYear > currentYear + 1 ||
      PTOEndYear > currentYear + 1
    ) {
      toast.error("Year requested for PTO is invalid");
      return;
    } else if (
      (PTOStartMonth > PTOEndMonth && PTOStartYear === PTOEndYear) ||
      (PTOStartMonth < currentMonth && PTOStartYear === currentYear) ||
      (PTOEndMonth < currentMonth && PTOEndYear === currentYear) ||
      (PTOStartMonth > currentMonth + 6 && PTOStartYear === currentYear) ||
      (PTOEndMonth > currentMonth + 6 && PTOEndYear === currentYear)
    ) {
      ...
      (PTOStartDay < currentDay &&
        PTOStartMonth === currentMonth &&
        PTOStartYear === currentYear) ||
      (PTOEndDay < currentDay &&
        PTOEndMonth === currentMonth &&
        PTOEndYear === currentYear)
    ) {
      ...
      ((PTOStartDay <= currentDay + 14 && PTOStartMonth === currentMonth) ||
        PTOEndDay - PTOStartDay > 14) &&
      PTOEndMonth === PTOStartMonth
    ) {
      ...
      (PTOEndDay <= currentDay + 14 && PTOEndMonth === currentMonth) ||
      (PTOEndDay < PTOStartDay && PTOEndMonth === PTOStartMonth)
    ) {
    ...
    if (
      Number(findEmployee[0].PTO) - Number(PTO) < 0 &&
      Number(PTO) >= Number(findEmployee[0].PTO)
    ) {
    ...
    holidayDates.forEach((holiday: string) => {
      ...
      if (
        (month === PTOStartMonth && date === PTOStartDay) ||
        (month === PTOEndMonth && date === PTOEndDay)
      ) {
    ...
    if (
      (martin.getMonth() === PTOStartMonth &&
        martin.getDate() === PTOStartDay) ||
      (martin.getMonth() === PTOEndMonth && martin.getDate() === PTOEndDay)
    ) {
      console.log("Martin Luther King Jr. Day");
    } else if (
      (labor.getMonth() === PTOStartMonth && labor.getDate() === PTOStartDay) ||
      (labor.getMonth() === PTOEndMonth && labor.getDate() === PTOEndDay)
    ) {
      console.log("Labor Day");
    } else if (
      (thanksgiving.getMonth() === PTOStartMonth &&
        thanksgiving.getDate() === PTOStartDay) ||
      (thanksgiving.getMonth() === PTOEndMonth &&
        thanksgiving.getDate() === PTOEndDay)
    ) {
...
async function ApprovePTO(props: Approve) {
  try {
    ...
    if (
      Number(findEmployee[0].PTO) - Number(props.PTO) >= 0 &&
      Number(props.PTO) &&
      Number(props.PTO) <= Number(findEmployee[0].PTO)
    ) {
      if (findEmployee.length) {
        if (findEmployee[0].requests) {
          findEmployee[0].requests.push(
            JSON.stringify({
        ...
        const data = {
          ...
          requests: findEmployee[0].requests
            ? findEmployee[0].requests
            : [
                JSON.stringify({
                  startDate: props.startDate,
                  endDate: props.endDate,
      ...
      toast.error(
        "Please fill out the PTO input with a valid value and try again!",
      );
    }
  } catch (err) {
    console.error(err);
...
export function RenderPTORequests(props: PTORequests) {
  const requests = props.PTORequests?.map((user: PTO) => {
    return (
      <ul key={user.$id} className="flex flex-col alignCenter userDisplays">
        ...
        <li>
          {user.PTOStartDate} - {user.PTOEndDate}
        </li>
        <li className="flex">
          <span>Hours: {user.PTO}</span>
          {Button({
            text: "",
            classNames: "fa-solid fa-check",
            handleButtonClick: () =>
              ApprovePTO({
                PTO: user.PTO,
                name: user.name,
                email: user.email,
                userID: user.userID,
                startDate: user.PTOStartDate,
                endDate: user.PTOEndDate,
                $id: user.$id,
              }),
          })}
          {Button({
            text: "",
            classNames: "fa-solid fa-xmark",
            handleButtonClick: () =>
              DenyPTO({
                PTO: user.PTO,
                name: user.name,
  ...
  return (
    <section className="flex flex-col alignCenter PTO">
      ...
      <PaginatedButtons
        currentPage={props.currentPTOPage}
        cartLength={props.PTORequests.length}
        setCurrentPage={(e: number) => props.setCurrentPTOPage(e)}
        rowsPerPage={props.rows}
      />

      <section>{requests.length ? requests : <h2>No PTO Requests</h2>}</section>

      {Button({
        text: "Show Employee Customization",
        handleButtonClick: () =>
          toggleDisplay(
            (e: boolean) => props.setPTODisplay(e),
            props.PTODisplay,
          ),
      })}
    </section>
    ...
    if (props.requests.length) {
      ...
      const test = props.requests.map((request: string, i: number) => {
        ...
        if (parse.status === "approved") {
          ...
          return (
            <div key={`${month}/${day}/${year}`}>
              <h2>
                Upcoming Approved PTO: {upcomingYear}/{upcomingMonth}/
                {upcomingDay}
              </h2>
            </div>
          );
...
export function RenderRequestHistory(props: History) {
  if (props.requests) {
    if (props.requests.length) {
      let i = 0;
      const requestDisplay = props.requests
        .map((request: string) => {
          const parse = JSON.parse(request);
          return (
            <section key={i++}>
      ...
      return (
        <section>
          <PaginatedButtons
            ...
            cartLength={props.requests.length}
